...
function BlockController(options) {
  ...
  this.btqheight = 520520;
  this.btqpreminewindow = 100;
  
  if(this.node.network.toString() == bitcore.Networks.testnet.toString()) {
      this.btqheight = 1259790;
  ...
  if (summaryCache) {
    ...
    self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      ...
      self.node.services.bitcoind.getBlockHeader(hash, function(err, blockHeader) {
        ...
        var confirmations = blockHeader.confirmations;//self.node.services.bitcoind.height - height + 1;
...
BlockController.prototype.getBlockReward = function(height) {
  ...
  // Premine subsidy.
  if(height >= this.btqheight 
		  && height < (this.btqheight + this.btqpreminewindow))
  {
      var subsidy = new BN((210000 / 100) * 1e8);
